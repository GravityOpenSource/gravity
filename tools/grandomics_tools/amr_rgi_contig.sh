...
#echo $num
awk 'BEGIN{FS="\t";OFS="\t"} {if(NR==1) {print "耐药基因/蛋白名称(Best_Hit_ARO)","抗性药物名称(Drug Class)","抗性机制(Resistance Mechanism)","耐药基因家族(AMR Gene Family)"}else{print $9,$15,$16,$17}}END{a=FNR-1;print "#总共检测到"a"个耐药基因"}' out.txt > amr.tsv
