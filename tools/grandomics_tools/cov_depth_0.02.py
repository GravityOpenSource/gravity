...
class DepthCoverageStat(object):
	...
	def process_data_v2(self):##2020/05/12 只输出一个coverage和一个depth文件即可
		coverage_genome = self.prefix+".mosdepth.global.dist.txt"##coverage
		per_base = self.prefix+".per-base.bed.gz"##depth
		
		df = pd.read_table(coverage_genome, sep="\t", header=None)
		# df_coverage = df[(df[0]=="total") & (df[1] <= 1000)]
		df_coverage = df[(df[0] != "total")]
		output = df_coverage.sort_values(by=1 , ascending=True)
		output.to_csv("out.coverage", sep="\t", header=0, index=0)#header=["Chrom","Depth","Coverage Percent"]
		
		self.get_depth(per_base, "out.depth")
		
	def get_depth(self,infile,outfile):
		fo=open(outfile,"w")
		# fo.write("Chrom	Coordinate	Depth\n")
		for line in gzip.open(infile):
			...
			if end - start > 1:
				for pos in range(start,end):
					fo.write(chrom+"\t"+str(pos)+"\t"+depth+"\n")
			else:
				fo.write(chrom+"\t"+str(start)+"\t"+depth+"\n")
		fo.close()
