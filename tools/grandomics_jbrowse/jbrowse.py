...
def metadata_from_node(node):
    ...
    metadata['dataset_edam_format'] = '<a target="_blank" href="http://edamontology.org/{0}">{1}</a>'.format(
    ...
    metadata[
        'history_display_name'] = '<a target="_blank" href="{galaxy}/history/view/{encoded_hist_id}">{hist_name}</a>'.format(
...
class JbrowseConnector(object):
    ...
    def add_bigwig_multiple(self, data, trackData, wiggleOpts, **kwargs):
        ...
        for idx, bw in enumerate(data):
            ...
            urls.append({"url": os.path.join('raw', trackData['label'] + '_' + str(idx) + '.bw'),
                         "name": str(idx + 1) + ' - ' + bw[0]})
    ...
    def process_annotations(self, track):
        ...
        outputTrackConfig = {
            ...
            'maxHeight': track['style'].get('maxHeight', '100000'),
            'category': category,
        }
...
def run_jbrowse(name, jbrowse, outdir, standalone):
    tree = ET.parse(name)
    root = tree.getroot()
